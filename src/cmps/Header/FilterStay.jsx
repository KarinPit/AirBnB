...
const handleStartDateChange = ({ start, end }) => {
  ...
  const handleEndDateChange = ({ end }) => {
    ...
    if (isValid(date)) {
      ...
      if (startDate && date && startDate >= date) {
        setStartDatePick(new Date(date.getTime() - 86400000));
      }
    } else {
      setEndDatePick(null);
    }
 ...
 const handleSelectRecentSearch = (search) => {
    ...
    updateCurrentOrder({ query: search.label, checkIn: checkInDate, checkOut: checkOutDate ,guestCounts: search.guests || '0'});
  };
